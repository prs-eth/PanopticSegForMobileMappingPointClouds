...
class MyPanopticTracker(SegmentationTracker):
    ...
    def track(self, model: model_interface.TrackerInterface, full_res=False, data=None, iou_threshold=0.25, track_instances=True, **kwargs):
        ...
        if torch.max(labels.instance_labels)>0:
            if len(outputs.embed_clusters)>0:
        ...
        #if not outputs.offset_clusters:
        #    return
            if len(outputs.offset_clusters)>0:
        ...
        if track_instances and len(outputs.offset_clusters)>0 and len(outputs.embed_clusters)>0:
        ...
        if self._test_area is None:
            ...
            #print(self._dataset.test_data_num_spheres)
        ...
        if originids.max() >= self._test_area[count_cloud].pos.shape[0]:
            raise ValueError("Origin ids are larger than the number of points in the original point cloud.")
        
        
        # Set predictions
        self._test_area[count_cloud].votes[originids] += outputs.semantic_logits
        self._test_area[count_cloud].prediction_count[originids] += 1
        #block merging for offsets and embedding features
        self._test_area[count_cloud].ins_pre_embed, self._test_area[count_cloud].max_instance_embed = self.block_merging(originids.cpu().numpy(), outputs.embed_pre.cpu().numpy(), self._test_area[count_cloud].ins_pre_embed.cpu().numpy(), self._test_area[count_cloud].max_instance_embed, model.get_opt_mergeTh())
        self._test_area[count_cloud].ins_pre_offset, self._test_area[count_cloud].max_instance_offset = self.block_merging(originids.cpu().numpy(), outputs.offset_pre.cpu().numpy(), self._test_area[count_cloud].ins_pre_offset.cpu().numpy(), self._test_area[count_cloud].max_instance_offset,  model.get_opt_mergeTh())
      	#return num_clusters, torch.from_numpy(labels)
        if count_sphere==self._dataset.test_data_num_spheres[count_cloud]:
            count_cloud+=1
            count_sphere=0
            ...
            for i, test_area_i in enumerate(self._test_area):
                ...
                if ply_output:
                    ...
                    '''self._dataset.to_ply(
                        ...
                        "vote1regularfull_{}.ply".format(i),
                    )'''
                    ...
                    #instance
                    has_prediction = test_area_i.ins_pre_embed != -1
                    ...
                    full_ins_pred_embed = test_area_i.ins_pre_embed[has_prediction][x_idx]

                    #full_ins_pred_embed2 = self._test_area.ins_pre_embed[has_prediction][assign_index2]

                    
                    has_prediction = test_area_i.ins_pre_offset != -1
                    ...
                    assign_index  = knn(test_area_i.pos[has_prediction], test_area_i.pos, k=1)
                    y_idx, x_idx = assign_index
                    full_ins_pred_offset = test_area_i.ins_pre_offset[has_prediction][x_idx]
                    ...
                    self._dataset.to_eval_ply(
                        ...
                        "Instance_Embed_results_forEval_{}.ply".format(i),
                    )
                    self._dataset.to_eval_ply(
                        test_area_i.pos,
                        full_ins_pred_offset.numpy(),
                        test_area_i.instance_labels,
                        "Instance_Offset_results_forEval_{}.ply".format(i),
                    )
                    ...
                    self._dataset.to_ins_ply(
                        test_area_i.pos[things_idx_embed],
                        full_ins_pred_embed[things_idx_embed].numpy(),
                        "Instance_Embed_results_withColor_{}.ply".format(i),
                    )
                    things_idx_offset = full_ins_pred_offset != -1
                    self._dataset.to_ins_ply(
                        test_area_i.pos[things_idx_offset],
                        full_ins_pred_offset[things_idx_offset].numpy(),
                        "Instance_Offset_results_withColor_{}.ply".format(i),
                    )
