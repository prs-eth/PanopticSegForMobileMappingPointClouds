...
from torch_points3d.datasets.segmentation.Italy import ItalySphere, ItalyCylinder, INV_OBJECT_LABEL
import torch_points3d.core.data_transform as cT
from torch_points3d.metrics.panoptic_tracker import PanopticTracker
from torch_points3d.metrics.panoptic_tracker_italy import MyPanopticTracker
...
def final_eval(pre_sem, pre_ins_embed, pre_ins_offset, gt_sem, gt_ins, file_path):
    NUM_CLASSES = 5
    NUM_CLASSES_count = 4
    #class index for instance segmenatation
    ins_classcount = [2,3,4] 
    #class index for stuff segmentation
    stuff_classcount = [1]
    #class index for semantic segmenatation
    sem_classcount = [1,2,3,4] 
...
class PanopticItalySphere(PanopticItalyBase, ItalySphere):
    def process(self):
        super().process()
...
class PanopticItalyCylinder(PanopticItalyBase, ItalyCylinder):
    def process(self):
        super().process()
...
class ItalyDataset(BaseDataset):
    ...
    def __init__(self, dataset_opt):
        ...
        dataset_cls = PanopticItalyCylinder if sampling_format == "cylinder" else PanopticItalySphere
        #if OmegaConf.is_list(self.dataset_opt.fold):
        if isinstance(self.dataset_opt.fold[0], int):
    ...
    @staticmethod
    def to_eval_ply(pos, pre_label, gt, file):
        """ Allows to save Italy predictions to disk for evaluation
    ...
    @staticmethod
    def to_ins_ply(pos, label, file):
        """ Allows to save Italy instance predictions to disk using random color
    ...
    def final_eval(pre_sem, pre_ins_embed, pre_ins_offset, gt_sem, gt_ins, file_path):
        ...
        final_eval(pre_sem, pre_ins_embed, pre_ins_offset, gt_sem, gt_ins, file_path)
